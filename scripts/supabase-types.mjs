...
const projectRef = 'ckrbsdzfzfkxxfhexbfk'
const targetPath = fileURLToPath(new URL('../src/supabase/database.types.ts', import.meta.url))
const checkOnly = process.argv.includes('--check')
const accessToken = process.env.SUPABASE_ACCESS_TOKEN

if (!accessToken) {
  console.error('SUPABASE_ACCESS_TOKEN is required. The type script will not read credentials from macOS Keychain.')
  process.exit(1)
}
...
let result
try {
  result = spawnSync('supabase', ['gen', 'types', 'typescript', '--project-id', projectRef, '--schema', 'public'], {
